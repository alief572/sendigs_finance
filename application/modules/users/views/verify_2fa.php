...
<div class="login-wrapper">
  <div class="login-card <?= $has_error ? 'has-error' : '' ?>">
    ...
    <div class="auth-instruction">
      Masukkan 6 digit kode OTP dari aplikasi <strong><a target="_blank" href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=id">Google Authenticator</a></strong> pada ponsel Anda.
    </div>

    <form method="post" action="<?= site_url('users/check_otp') ?>" id="frm_verify_2fa" autocomplete="off">
      <div class="form-group-custom">
        <label class="form-label-custom" for="otp">Kode Keamanan OTP</label>
        <div class="input-wrapper">
          <input 
            type="text" 
            name="otp" 
            id="otp" 
            class="form-input-custom otp-input" 
            placeholder="······" 
            maxlength="6" 
            pattern="[0-9]*" 
            inputmode="numeric" 
      ...
      <button type="submit" class="btn-submit-login" id="btnSubmitOtp" name="login">
...
<script>
  $(document).ready(function() {
    ...
    $('#frm_verify_2fa').on('submit', function() {
      var btn = $('#btnSubmitOtp');
      btn.css({'pointer-events': 'none', 'opacity': '0.85'}).html('<i class="fa fa-spinner fa-spin"></i> <span>Memverifikasi...</span>');
    });
  });
</script>