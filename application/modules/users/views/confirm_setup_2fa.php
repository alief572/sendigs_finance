...
<div class="login-wrapper">
  <div class="login-card <?= $has_error ? 'has-error' : '' ?>">
    <div class="login-header">
      ...
      <h1 class="login-title">Konfirmasi Setup 2FA</h1>
      <p class="login-subtitle"><?= !empty($idt->nm_perusahaan) ? htmlspecialchars($idt->nm_perusahaan) : 'SENDIGS SS' ?></p>
    </div>

    <div class="auth-instruction">
      Masukkan password akun Anda untuk mengonfirmasi dan melanjutkan pengaturan Google Authenticator.
    </div>

    <form action="<?= base_url($this->uri->uri_string()) ?>" method="POST" id="frm_confirm_setup" autocomplete="off">
      ...
      <button type="submit" class="btn-submit-login" id="btnSubmitConfirm">
...
<script>
  $(document).ready(function() {
    ...
    $('#frm_confirm_setup').on('submit', function() {
