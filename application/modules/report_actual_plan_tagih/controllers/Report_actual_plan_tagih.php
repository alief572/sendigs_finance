...
class Report_actual_plan_tagih extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->_sync_view_schema();
    }

    private function _sync_view_schema()
    {
        $dberp = defined('DBERP') ? DBERP : $this->db->database;
        $dbcnl = defined('DBCNL') ? DBCNL : 'db_consultant_new';

        $sql = "CREATE OR REPLACE VIEW `view_rekap_actual_plan_tagih_dev` AS
        SELECT
        ...
        JOIN `{$dberp}`.`kons_tr_plan_tagih_header` `pth`
            ON `pth`.`id_spk_penawaran` = `a`.`id_spk_penawaran`
        LEFT JOIN `{$dbcnl}`.`kons_tr_penawaran` `b`
            ON `b`.`id_quotation` = `a`.`id_penawaran`
        LEFT JOIN `{$dbcnl}`.`kons_tr_company` `c`
            ON `c`.`id` = `b`.`company`
        LEFT JOIN `{$dbcnl}`.`kons_master_konsultasi_header` `d`
        ...
        LEFT JOIN (
            ...
            FROM `{$dberp}`.`kons_tr_plan_tagih_detail`
        ...
        JOIN (
            ...
            FROM (
                ...
                FROM `{$dberp}`.`kons_tr_actual_plan_tagih` `act`
                JOIN `{$dberp}`.`kons_tr_plan_tagih_detail` `det`
            ...
            FROM `{$dberp}`.`kons_tr_plan_tagih_detail` `p`
            WHERE EXISTS(
                SELECT 1
                FROM `{$dberp}`.`kons_tr_actual_plan_tagih` `act`
