-- Alter view_rekap_actual_plan_tagih
...
CREATE OR REPLACE VIEW `view_rekap_actual_plan_tagih` AS
SELECT
    ...
    COALESCE(SUM(CASE WHEN `mt`.`bln` = 5 THEN `mt`.`nominal` END), 0) AS `may_val`,
    ...
    COALESCE(SUM(CASE WHEN `mt`.`bln` = 12 THEN `mt`.`nominal` END), 0) AS `dec_val`
FROM `db_consultant_new`.`kons_tr_spk_penawaran` `a`
JOIN `kons_tr_plan_tagih_header` `pth`
...
LEFT JOIN (
    ...
    FROM (
        ...
        FROM `kons_tr_actual_plan_tagih` `act`
        JOIN `kons_tr_plan_tagih_detail` `det`
