...
class Report_jurnal_payment_model extends BF_Model
{
    ...
    public function get_data_jurnal()
    {
        ...
        // Base filter criteria (sts = '1' untuk data yang SUDAH posting)
        ...
        foreach ($get_data as $item) {
            ...
            // Badge styling untuk kategori transaksi dengan warna yang senada
            ...
            $action_btn = '<button type="button" class="btn btn-sm btn-info view_jurnal" data-no_transaksi="' . htmlspecialchars($item->no_transaksi, ENT_QUOTES) . '" data-jenis_transaksi="' . htmlspecialchars($item->jenis_transaksi, ENT_QUOTES) . '" title="View Detail Jurnal" style="border-radius: 6px; padding: 4px 10px; font-weight: 600;"><i class="fa fa-eye"></i> View</button>';
    ...
    public function get_detail_jurnal($no_transaksi, $jenis_transaksi)
    {
        $this->db->select('a.*');
        $this->db->from('tr_jurnal a');
        ...
        $this->db->where('a.sts', '1');
        $this->db->where('(a.debit > 0 OR a.kredit > 0)');
