...
<script>
    ...
    $(document).on('submit', '#frm_data', function(e) {
        ...
        // Validasi balance jurnal: total debit harus sama dengan total kredit
        var total_debit_jurnal = parseFloat($('input[name="total_debit_jurnal"]').val()) || 0;
        var total_kredit_jurnal = parseFloat($('input[name="total_kredit_jurnal"]').val()) || 0;

        if (total_debit_jurnal !== total_kredit_jurnal) {
            swal({
                type: 'error',
                title: 'Jurnal Tidak Balance !',
                text: 'Total Debit (' + number_format(total_debit_jurnal) + ') tidak sama dengan Total Kredit (' + number_format(total_kredit_jurnal) + '). Data tidak dapat disimpan.',
