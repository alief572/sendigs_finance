...
class Actual_plan_tagih_model extends BF_Model
{
    ...
    public function get_actual_plan_tagih()
    {
        ...
        $this->db->select('a.*, b.nm_customer, b.nm_project, b.nm_project_leader, d.created_date, d.id as id_actual');
        ...
        if ($bulan == 'macet') {
            ...
            // $this->db->group_start();
            $this->db->where('DATE_FORMAT(IF(d.tanggal_actual_plan_tagih NOT LIKE "%0000-00-00%", d.tanggal_actual_plan_tagih, a.tgl_plan_tagih), "%Y") =', $tahun);
            $this->db->where('DATE_FORMAT(IF(d.tanggal_actual_plan_tagih NOT LIKE "%0000-00-00%", d.tanggal_actual_plan_tagih, a.tgl_plan_tagih), "%m") =', sprintf('%02s', $bulan));
            // $this->db->group_end();
            if (!empty($status)) {
                if ($status == '1' || $status == '2') {
                    $this->db->where('d.tagih_mundur', $status);
        ...
        foreach ($get_data_all->result() as $item_all) {
            ...
            if (!empty($item_all->created_date)) {
                ...
                $this->db->where('a.id', $item_all->id_actual);
