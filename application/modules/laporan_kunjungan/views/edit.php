...
$is_final = (isset($visit->status) && $visit->status === 'final');
$disabled_attr = $is_final ? 'disabled' : '';
?>
...
<style>
    ...
    .btn-remove-ap { color: #d9534f; }
    .btn-add-ap { margin-top: 5px; }
    .final-notice { background: #d9edf7; border: 1px solid #bce8f1; color: #31708f; padding: 15px; border-radius: 5px; margin-bottom: 20px; text-align: center; }
    .final-notice i { margin-right: 5px; }
</style>

<?php if ($is_final): ?>
<div class="final-notice">
    <i class="fa fa-lock"></i>
    <strong>Laporan ini sudah berstatus Final dan tidak dapat diedit.</strong>
</div>
<?php endif; ?>
...
<div class="box">
    <div class="box-header">
        <h3 class="box-title">Edit Laporan Kunjungan</h3>
    </div>
    <div class="box-body">
        <div class="row">
            <div class="col-md-6">
...
<!-- Visit Session Section (pre-populated, editable time inputs for draft) -->
<div class="box">
    <div class="box-header">
        ...
        <div class="row">
            <div class="col-md-4 text-center">
                ...
                <button type="button" class="btn btn-success btn-sm" id="btn-start"
                    <?php echo $visit->start_time ? 'disabled' : ''; ?>>
            ...
            <div class="col-md-4 text-center">
                ...
                <button type="button" class="btn btn-danger btn-sm" id="btn-finish"
                    <?php echo (!$visit->start_time || $visit->finish_time) ? 'disabled' : ''; ?>>
            ...
            <div class="col-md-4 text-center">
                ...
                <div style="margin-top:5px;">
                    ...
                    <span id="mandays-terpakai-display" class="duration-display">
                        <?php echo $visit->mandays_used ? number_format($visit->mandays_used, 2) : '-'; ?>
                    </span>
                </div>
            </div>
        </div>
    ...
    <div class="box-body">
        <?php if ($kegiatan_list && count($kegiatan_list) > 0): ?>
            <div class="form-group">
                ...
                <?php
                // Build array of selected kegiatan IDs from visit data
                $selected_kegiatan_ids = [];
                if (isset($visit->kegiatan) && is_array($visit->kegiatan)) {
                    foreach ($visit->kegiatan as $vk) {
                        if (!$vk->is_custom && $vk->id_aktifitas) {
                            $selected_kegiatan_ids[] = $vk->id_aktifitas;
                        }
                    }
                }
                ?>
                    ...
                    <div class="checkbox">
                        <label>
                            <input type="checkbox" class="kegiatan-checkbox"
                                   ...
                                   <?php echo in_array($kegiatan->id_aktifitas, $selected_kegiatan_ids) ? 'checked' : ''; ?>
        ...
        <div id="kegiatan-action-plans-container" class="kegiatan-section">
            <!-- Dynamically populated from saved data -->
        </div>
    </div>
</div>
    ...
    <div class="box-body">
        <div class="form-group">
            <textarea class="form-control" id="potensi-improvement" rows="5"
                      maxlength="2000" placeholder="Masukkan potensi improvement (opsional, maks 2000 karakter)"
                      <?php echo $disabled_attr; ?>><?php echo htmlspecialchars($visit->potensi_improvement ?? ''); ?></textarea>
            <div class="char-counter"><span id="potensi-counter"><?php echo mb_strlen($visit->potensi_improvement ?? ''); ?></span>/2000</div>
        </div>
    </div>
</div>
    ...
    <div class="box-body">
        <div class="form-group">
            <textarea class="form-control" id="hasil-improvement" rows="5"
                      maxlength="2000" placeholder="Masukkan hasil improvement (opsional, maks 2000 karakter)"
                      <?php echo $disabled_attr; ?>><?php echo htmlspecialchars($visit->hasil_improvement ?? ''); ?></textarea>
            <div class="char-counter"><span id="hasil-counter"><?php echo mb_strlen($visit->hasil_improvement ?? ''); ?></span>/2000</div>
        </div>
    </div>
</div>

<!-- Save Buttons (hidden if final) -->
<?php if (!$is_final): ?>
<div class="box">
    <div class="box-body text-center">
        ...
        <button type="button" class="btn btn-warning" id="btn-update-draft" style="margin-left:10px;">
...
$(document).ready(function() {
    ...
    var visitDate = '<?php echo htmlspecialchars($visit->visit_date); ?>';
    var idSpk = '<?php echo addslashes($id_spk); ?>';
    var isFinal = <?php echo $is_final ? 'true' : 'false'; ?>;
    var selectedKegiatan = []; // Array of {id, nama, is_custom, db_kegiatan_id, action_plans: [...]}

    // ===== Pre-populate kegiatan from saved data =====
    <?php if (isset($visit->kegiatan) && is_array($visit->kegiatan)): ?>
    <?php foreach ($visit->kegiatan as $vk): ?>
    selectedKegiatan.push({
        id: <?php echo json_encode($vk->is_custom ? 'custom_' . $vk->id : $vk->id_aktifitas); ?>,
        nama: <?php echo json_encode($vk->nama_kegiatan); ?>,
        is_custom: <?php echo $vk->is_custom ? 'true' : 'false'; ?>,
        db_kegiatan_id: <?php echo (int) $vk->id; ?>,
        action_plans: [
            <?php if (isset($vk->action_plans) && is_array($vk->action_plans)): ?>
            <?php foreach ($vk->action_plans as $ap): ?>
            {
                id: <?php echo (int) $ap->id; ?>,
                description: <?php echo json_encode($ap->description); ?>,
                pic: <?php echo json_encode($ap->pic); ?>,
                due_date: <?php echo json_encode($ap->due_date); ?>,
                status: <?php echo json_encode($ap->status); ?>
            },
            <?php endforeach; ?>
            <?php endif; ?>
        ]
    ...
    // Ensure each kegiatan has at least one action plan
    $.each(selectedKegiatan, function(idx, k) {
        if (k.action_plans.length === 0) {
            k.action_plans.push(createEmptyActionPlan());
        }
    });
    ...
    // If final, stop here — no interaction needed
    if (isFinal) {
        return;
    }
    ...
    function renderKegiatanBlocks() {
        ...
        $.each(selectedKegiatan, function(kIdx, kegiatan) {
            ...
            // Add action plan button (max 50) — only if not final
            if (!isFinal && kegiatan.action_plans.length < 50) {
    ...
    function buildActionPlanRow(kIdx, apIdx, ap, totalAps) {
        ...
        var disabledStr = isFinal ? ' disabled' : '';

        var html = '<div class="row">';
        html += '<div class="col-md-4">';
        html += '<div class="form-group"><label>Deskripsi <span class="text-danger">*</span></label>';
        html += '<input type="text" class="form-control ap-description" maxlength="500" value="' + escapeAttr(ap.description) + '" placeholder="Deskripsi action plan (maks 500)"' + disabledStr + '>';
        html += '</div></div>';

        html += '<div class="col-md-2">';
        html += '<div class="form-group"><label>PIC <span class="text-danger">*</span></label>';
        html += '<input type="text" class="form-control ap-pic" maxlength="100" value="' + escapeAttr(ap.pic) + '" placeholder="PIC (maks 100)"' + disabledStr + '>';
        html += '</div></div>';

        html += '<div class="col-md-2">';
        html += '<div class="form-group"><label>Due Date <span class="text-danger">*</span></label>';
        html += '<input type="date" class="form-control ap-due-date" value="' + escapeAttr(ap.due_date) + '" min="' + visitDate + '"' + disabledStr + '>';
        html += '</div></div>';

        html += '<div class="col-md-2">';
        html += '<div class="form-group"><label>Status</label>';
        html += '<select class="form-control ap-status"' + disabledStr + '>';
        ...
        if (!isFinal && totalAps > 1) {
            html += '<button type="button" class="btn btn-xs btn-danger btn-remove-ap" data-kegiatan-idx="' + kIdx + '" data-ap-idx="' + apIdx + '"><i class="fa fa-trash"></i> Hapus</button>';
        }
        html += '</div></div>';
        html += '</div>';
    ...
    $(document).on('click', '.btn-remove-ap', function() {
        ...
        if (selectedKegiatan[kIdx].action_plans.length <= 1) {
    ...
    function validateForm(isFinalSave) {
        var errors = [];
        syncActionPlanData();

        // Validate start/finish time
        var startTimeVal = startTime;
        var finishTimeVal = finishTime;

        if (isFinalSave) {
            if (!startTimeVal) {
                errors.push('Start time wajib diisi.');
            }
            if (!finishTimeVal) {
                errors.push('Finish time wajib diisi.');
            }
            if (startTimeVal && finishTimeVal && startTimeVal >= finishTimeVal) {
    ...
    // ===== Update Draft =====
    $('#btn-update-draft').on('click', function() {
        ...
        $.ajax({
            url: '<?php echo base_url('laporan_kunjungan/update_draft'); ?>',
            type: 'POST',
            dataType: 'json',
            ...
            success: function(response) {
                ...
                if (response.status) {
                    ...
                    alert(response.message || 'Gagal memperbarui draft.');
                }
            },
            error: function() {
                btn.prop('disabled', false);
    ...
    $('#btn-save-final').on('click', function() {
        ...
        $.ajax({
            url: '<?php echo base_url('laporan_kunjungan/update_final'); ?>',
            type: 'POST',
            dataType: 'json',
            contentType: 'application/json',
            data: JSON.stringify(formData),
